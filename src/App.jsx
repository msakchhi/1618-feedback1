...
import { Heart, TrendingUp, AlertCircle, CheckCircle, MessageCircle, Lock } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
        <div className="text-center mb-6">
          <Lock className="mx-auto text-red-600 mb-3" size={40} />
          <h2 className="text-2xl font-bold text-gray-800">Staff Only</h2>
          <p className="text-gray-500 text-sm mt-1">Enter your password to view the dashboard</p>
        </div>
        <form onSubmit={handlePasswordSubmit} className="space-y-4">
          ...
          />
          {passwordError && (
            <p className="text-red-600 text-sm text-center">❌ Incorrect password. Try again.</p>
          )}
          <button
            type="submit"
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSurveySubmit} className="space-y-6">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-3">How was your meal? ⭐</label>
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button key={star} type="button"
                          onClick={() => setFormData({ ...formData, rating: star })}
                          className={`text-4xl transition transform hover:scale-110 ${star <= formData.rating ? "opacity-100" : "opacity-30"}`}
                        >⭐</button>
                      ))}
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              {feedbackList.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-8">No feedback yet — share the survey link to get started! 🎉</p>
              ) : (
                <div className="space-y-4">
                  {feedbackList.map((item) => (
                    <div key={item.id} className="flex gap-3 pb-4 border-b border-gray-100 last:border-0">
                      <div className="flex-shrink-0 mt-1">
                        {item.segment === "promoter" ? <CheckCircle className="text-green-500" size={20} />
                          : item.segment === "detractor" ? <AlertCircle className="text-red-500" size={20} />
                      ...
                      <div className="flex-1">
                        <div className="flex gap-2 mb-1 flex-wrap">
                          ...
                          {item.firstVisit && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">First visit</span>}
